...
# Sort the characters in s such that if the character is present in t then it is sorted
# according to the order in t and other characters are sorted alphabetically after the ones found in t.
#
# Examples
...
def custom_sort(t, s):
    ...
    return ''.join(sorted(s, key=lambda c: pos.get(c, ord(c))))
